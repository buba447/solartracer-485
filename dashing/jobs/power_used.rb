...
SCHEDULER.every '1m', :first_in => 0 do |job|
  ...
  # Mysql query
  sql = "select daily_kw_used, UNIX_TIMESTAMP(date) FROM daily_kw_used WHERE date >= NOW() - INTERVAL 1 DAY"
  ...
  send_event('power_used', points: items)
